...
This tool facilitates migrating GCE VM protection from Management Console Backup Plans (SLT) to Backup Vault Backup Plans. It does **not** migrate existing backups. The process uses an **overlap and cutover** strategy:
...
Migration is **not supported** if your current Backup Plan (SLT) or VM setup includes any of the following:
...
The tools will check for these limitations at both the Backup Plan (SLT) and individual VM levels.
...
* A pre-existing **Backup Plan** configured to use the Backup Vault. This Backup Plan is created in the Google Cloud Console and can be equivalent to the Backup Plan (SLT) defined in the Management Console.  
...
The tool is provided as a single executable: [`backupdr_gcevm_migration_tool`](https://github.com/premkjain/backupdr-gcevm-migration).
...
* `--slt_name` (Required): Name of the source Backup Plan (SLT) in the Management Console.  
...
* `--workload_projects` (Required): Comma-separated list of project IDs containing GCE VMs, or "`all`" for all projects associated with the Backup Plan (SLT).  
...
* `--slt_name` (Required): Name of the source Backup Plan (SLT).  
* `--management_server_url` (Required): URL of the Backup and DR Management Console.  
...
**Verify:** Check the Management Console to confirm eligible instances are no longer protected by the Backup Plan (SLT).
